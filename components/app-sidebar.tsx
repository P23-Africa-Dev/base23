...
const PROFILE_SHOWCASE_ITEMS: ProfileNavItem[] = [
    { name: 'Personal Profile', href: '/profile?tab=personal', icon: <IoPerson className="h-5 w-5" /> },
    { name: 'Company Profile', href: '/profile?tab=company', icon: <HiOutlineUserGroup className="h-5 w-5" /> },
    { name: 'Subscription', href: '/profile?tab=subscription', icon: <MdOutlineWorkspacePremium className="h-5 w-5" /> },
    { name: 'Settings', href: '/profile?tab=settings', icon: <SlSettings className="h-5 w-5" /> },
];
...
const NAV_ITEMS: NavItem[] = [
    ...
    { name: 'Deal Card', icon: images.dealCardDesktop, href: '/dealcard' },
    { name: 'Leads', icon: images.searchList1, href: '/leads' },
];
const MOBILE_NAV_ITEMS: MobileNavItem[] = [
    { name: 'Dashboard', icon: images.dashboardIcon, activeIcon: images.activeDashboard, href: '/dashboard' },
    { name: 'Messages', icon: images.bubbleChat, activeIcon: images.activeChat, href: '/message' },
...
const userAccountItems: NavItem[] = [
    ...
    { name: 'Notifications', icon: `${images.notificationSidebarIcon}`, href: '#notifications' },
...
export const AppSidebar: React.FC = () => {
    ...
    const [selectedDealCardForModal, setSelectedDealCardForModal] = useState<any>(null);
    ...
    const [singleDealCardForPopup, setSingleDealCardForPopup] = useState<any>(null);
    const [singleDealCardStatus, setSingleDealCardStatus] = useState<'pending' | 'viewed' | 'accepted' | 'rejected'>('pending');

    // Received deal cards state (for floating popup across all pages)
    const [receivedDealCards, setReceivedDealCards] = useState<ReceivedDealCard[]>([]);
    ...
    const handleReceivedDealCardClick = (dealCardData: any, status: 'pending' | 'viewed' | 'accepted' | 'rejected') => {
        if (dealCardData) {
    ...
    useEffect(() => {
        ...
        // Update count every 30 seconds
        const notificationInterval = setInterval(fetchUnreadNotificationsCount, 30000);
    ...
    return (
        <div className="">
            {/* Desktop Navigation */}
            <aside
                className={`sticky top-0 left-0 z-[2] no-scrollbar hidden h-screen overflow-x-hidden overflow-y-auto transition-all duration-400 outline-none select-none lg:block ${open ? 'w-56' : 'w-20 overflow-x-hidden'} ${isLeadsPage
                ...
                <div className="flex h-full flex-col justify-between">
                                ...
                                return (
                                    <li key={item.name} className="relative">
                                        <Link
                                            ...
                                            className={`relative z-[1] flex cursor-pointer items-center transition-all duration-300 ${isActive ? 'font-bold' : 'text-gray-400 hover:text-white'
                                                }`}
                                            aria-current={isActive ? 'page' : undefined}
                                        >
                                            {/* Highlight background */}
                                            <div
                                                className={`absolute top-0 left-0 z-0 h-full w-[250px] transform bg-cover bg-center bg-no-repeat transition-all duration-300 ${isActive ? `active-link-bg -translate-x-3 rounded-l-full` : ''
                                                    }`}
                                            ></div>
                                            ...
                                            {/* Icon + Label */}
                                            <div
                                                className={`relative flex w-full items-center rounded-lg py-2 transition-colors duration-300 ${isActive
                                                    ? 'py-2.5 font-bold text-deepBlack hover:text-deepBlack/80'
                                                    : 'font-light text-white hover:text-white/80'
                                                    }`}
                                            >
                                                <div
                                                    className={`mr-3 flex h-8.5 w-8.5 flex-shrink-0 items-center justify-center rounded-full ${isLeadsPage
                                                        ? isActive
                                                ...
                                                <span
                                                    className={`overflow-hidden whitespace-nowrap transition-all duration-400 ${open ? 'ml-1.5 max-w-[120px] opacity-100' : 'ml-0 max-w-0 opacity-0'
                                                        }`}
                                                >
                                                    {item.name}
                                                </span>
                    ...
                    <div className="relative px-5">
                        ...
                        <div onClick={() => setProfileOpen(!profileOpen)} className="mb-4 flex cursor-pointer items-center space-x-2">
                            ...
                            >
                                <h3 className={`truncate text-[12px] font-bold ${isLeadsPage ? 'text-secondaryWhite' : ''} `}>{auth.user.name}</h3>
                                ...
                                item.name === 'Notifications' ? (
                                    ...
                                    >
                                        <div className="relative flex items-center rounded-lg transition-colors duration-400">
                                            ...
                                            <span
                                                className={`overflow-hidden text-[13px] whitespace-nowrap transition-all duration-300 ${open ? 'ml-2 max-w-[120px] opacity-100' : 'ml-0 max-w-0 opacity-0'
                                                    }`}
                                            >
                                                {item.name}
                                            </span>
            ...
            {(() => {
                ...
                const hideForMessageSingle = pathname === '/message/single' && search.length > 0;
                                        ...
                                        {MOBILE_NAV_ITEMS.map((item, index) => {
                                            ...
                                            return (
                                                <Link
                                                    ...
                                                    href={item.href}
                                                    onClick={() => setActivePath(item.href)}
                                                    className={`flex items-center justify-center rounded-full transition-all duration-300 ${isMiddle ? '-mt-10 h-18 w-18 border bg-white shadow-[inset_0_4px_6px_rgba(0,0,0,0.5)] md:-mt-24 md:h-26 md:w-26' : 'h-12 w-12'} ${isActive && isLeadsPage
            ...
            <AnimatePresence>
                {profileOpen && (
                    ...
                    >
                        <div className={`absolute top-[65%] left-[-8px] h-4 w-4 rotate-45  ${isLeadsPage ? 'bg-[#215568]' : 'bg-deepBlack'}`}></div>
                        ...
                        <ul className="mb-2 space-y-1 border-b border-secondaryWhite pb-2">
                            {PROFILE_SHOWCASE_ITEMS.map((item) => {
                                ...
                                if (activePath === '/profile') {
                                    if (item.name === 'Personal Profile' && currentTab === 'personal') isActive = true;
                                    if (item.name === 'Company Profile' && currentTab === 'company') isActive = true;
                                    if (item.name === 'Subscription and Coins' && currentTab === 'subscription') isActive = true;
                                    if (item.name === 'Settings' && currentTab === 'settings') isActive = true;
                                }
                                ...
                                return (
                                    <li key={item.name} className="relative">
                                        <Link
                                            ...
                                            className={`relative z-[1] flex cursor-pointer items-center rounded-xl transition-all duration-300 ${isActive ? 'font-bold text-deepBlack' : 'text-white/80 hover:bg-secondaryWhite hover:text-[#0b1727]'
                                                }`}
                                            aria-current={isActive ? 'page' : undefined}
                                        >
                                            {/* highlight background */}
                                            ...
                                            {/* Icon + Label */}
                                            <div
                                                className={`relative flex w-full items-center rounded-lg px-2 py-2 font-bold transition-colors duration-300 ${isActive ? 'text-deepBlack' : 'text-white hover:text-deepBlack'
                                                    }`}
                                            >
                                                <div
                                                    className={`mr-3 flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full ${!isActive && isLeadsPage
                                ...
                                <div className="p-6">
                                    ...
                                    <div className="mb-5 flex items-center gap-4">
                                        <div
                                            ...
                                            style={{
                                                backgroundImage: selectedDealCardForModal.recipient?.profile_picture
                                        ...
                                        <div className="flex-1">
                                            <p className="font-montserrat font-semibold text-gray-900">
                                                Sent to: {selectedDealCardForModal.recipient?.name || 'Unknown'}
                                            </p>
                                            <p className="text-sm text-gray-500">
                                                {selectedDealCardForModal.recipient?.position}
                                                {selectedDealCardForModal.recipient?.company_name && ` at ${selectedDealCardForModal.recipient.company_name}`}
                                            </p>
                                        </div>
                                        {/* Status badge */}
                                    ...
                                    <p className="mt-4 text-center text-sm text-gray-500">
                                        Sent on {new Date(selectedDealCardForModal.created_at).toLocaleDateString()}
                                    </p>
                                </div>
                            </div>
                        </motion.div>
