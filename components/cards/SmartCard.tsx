  ...
  return (
    <Card className="relative flex h-[202px] w-full max-w-[250px] flex-col justify-end overflow-hidden rounded-xl shadow-lg">
      ...
      <div className="relative -bottom-7 z-20 bg-[#FFFFFFCC]/80 p-4">
        ...
        <p className="text-xs font-medium text-darkBlue opacity-90">
          {match.title}
        </p>
  ...
  const fetchMatches = useCallback(async () => {
    ...
    try {
      ...
      if (response.success && response.matches && response.matches.length > 0) {
        ...
        const formattedMatches: SmartMatch[] = response.matches.map(
          (match: SmartMatchUser) => ({
            id: match.id,
            name: match.name,
  ...
  if (matches.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-8 px-4 text-center">
        ...
        <h3 className="text-sm font-semibold text-darkBlue mb-1">
          No One Needs Your Help Yet
        </h3>
        <p className="text-xs text-gray-500 max-w-[200px]">
          When users set up their preferences, you'll see those whose problems
