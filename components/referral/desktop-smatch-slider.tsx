...
// Empty placeholder data for when there are no matches
const EMPTY_PLACEHOLDER: SliderConnection = {
    id: 0,
    name: '',
    role: '',
    company: '',
...
export default function ReferralCardSlider({ data, onMatch, showEmpty = false }: Props) {
                ...
                <AnimatePresence mode="popLayout" initial={false}>
                    {SLOT_OFFSETS.map((slot) => {
                        ...
                        return (
                            ...
                            >
                                <CardSlot slot={slot} user={user} onMatch={() => onMatch(user)} onCardClick={() => handleCardClick(user)} />
                            </motion.div>
                        );
                    })}
                </AnimatePresence>
...
const slotConfig: Record<number, SlotConfig> = {
    ...
    '0': {
        ...
        buttonClass: 'px-3 py-2 text-[9px]',
    },
    '1': {
        cardSize: 'h-[290px] w-[220px]',
        imageRadius: 'rounded-2xl',
...
function CardSlot({ slot, user, onMatch, onCardClick }: CardSlotProps) {
    ...
    return (
        ...
        >
            <div className="relative flex h-full w-full flex-col gap-3">
                {/* IMAGE */}
                <div className={`relative h-screen w-full overflow-hidden ${config.imageRadius}`}>
                    <img
                ...
                <div className="flex flex-col items-center gap-0.5 px-4">
                    <div className="flex items-center gap-1.5">
                        <div className={`flex ${config.iconSize} items-center justify-center rounded-full bg-[#193E47]`}>
                            <div className={`relative ${config.iconInnerSize}`}>
                                <img src={images.aiMagic} className="absolute inset-0 h-full w-full object-contain" alt="" />
                            </div>
                        </div>
                        ...
                        <div className={`font-extrabold ${config.compatibilityText}`}>{user.compatibility}%</div>
                        ...
                        <button
                            ...
                            className={`ml-1.5 rounded-full bg-darkBlue font-semibold whitespace-nowrap text-white shadow-[1px_2px_4px_-1px_rgba(0,0,0,0.2)] pointer-events-auto relative z-30 ${config.buttonClass}`}
                        >
                            Match us!
                        </button>
                    </div>
...
function EmptyCardSlot({ slot }: EmptyCardSlotProps) {
    ...
    const iconSize = slot === 0 ? 'w-20 h-20' : slot === -1 || slot === 1 ? 'w-16 h-16' : 'w-12 h-12';
    ...
    return (
        ...
        >
            <div className="relative flex h-full w-full flex-col gap-3">
                ...
                >
                    <div className={`${iconSize} flex items-center justify-center rounded-full bg-gray-300`}>
                ...
                <div className="flex flex-col items-center gap-0.5 px-4">
                    <div className="flex items-center gap-1.5">
                        <div className={`flex ${config.iconSize} items-center justify-center rounded-full bg-gray-300`}>
                            <div className={`relative ${config.iconInnerSize}`}>
                                <img src={images.aiMagic} className="absolute inset-0 h-full w-full object-contain opacity-50" alt="" />
                            </div>
                        </div>
                        ...
                        <div className={`font-extrabold ${config.compatibilityText} text-gray-400`}>—%</div>
