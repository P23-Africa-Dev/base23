...
const SMART_MATCH_CARDS: SmartMatchCard[] = [
    { id: 1, size: 'sm', image: images.man1, compatibility: 78 },
    { id: 2, size: 'md', image: images.man2, compatibility: 84 },
    { id: 3, size: 'lg', image: images.man3, name: 'Jamal Agoro', title: 'CTO AfriLaw', compatibility: 92 },
    { id: 4, size: 'md', image: images.man4, compatibility: 81 },
    { id: 5, size: 'sm', image: images.man5, compatibility: 75 },
];

const CARD_SIZE_CLASSES = {
    sm: { wrapper: 'h-[240px]', image: 'w-[140px] rounded-2xl', dot: 'h-[20px] w-[20px]' },
    md: { wrapper: 'h-[320px]', image: 'w-[185px] rounded-2xl', dot: 'h-[26px] w-[26px]' },
    lg: { wrapper: 'h-[400px]', image: 'w-full rounded-3xl', dot: 'hidden' },
} as const;
...
export function SmartMatchCards() {
    ...
    return (
        <>
                ...
                <div className="flex gap-5">
                    {SMART_MATCH_CARDS.map((card) => {
                        ...
                        return (
                            <div
                                key={card.id}
                                className={`flex ${size.wrapper} flex-col items-center justify-center rounded-4xl bg-white p-3 shadow-[1px_3px_5px_-1px_rgba(0,0,0,0.4),-2px_3px_5px_-1px_rgba(0,0,0,0.4)]`}
                            >
                                        ...
                                        <>
                                            ...
                                            <div className="flex w-full items-center gap-2 px-4">
                                                <div className="flex h-[30px] w-[30px] items-center justify-center rounded-full bg-[#193E47]">
                                                    <img src={images.aiMagic} className="h-5 w-5" alt="" />
                                                </div>
                                                <div className="leading-3">
                                                    <h5 className="text-[7.4px]">Smart Matching</h5>
                                                    <p className="text-[10px] font-bold">Compatibility</p>
                                                </div>
                                                <div className="text-[17.8px] font-extrabold">{card.compatibility}%</div>
                                                <button
            ...
            <MatchSidebar
                open={!!activeCard}
                user={activeCard ? { id: activeCard.id, name: activeCard.name || '', role: activeCard.title, image: activeCard.image } : null}
