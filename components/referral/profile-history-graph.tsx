...
const MatchingHistoryChart = () => {
  ...
  return (
    <div className="h-18 pb-2 w-full flex flex-col items-center justify-end relative rounded-2xl bg-white shadow-[1px_3px_5px_-1px_rgba(0,0,0,0.2),-2px_3px_5px_-1px_rgba(0,0,0,0.2)]">
      {/* Header */}
      <h4 className="mb-2 absolute top-2.5 left-3.25 text-[8px] font-semibold text-gray-800">
        Matching History
      </h4>
      ...
      <div className="mt-2 flex justify-center gap-2 text-xs">
        {visibleMonths.map((item) => (
          <span
            key={item.month}
            className={`px-0.75 py-px font-medium text-[6px] transition-all ${item.isCurrent ? "rounded-full bg-[#978FED] text-white" : "text-gray-400"}`}
          >
            {item.month}
          </span>
        ))}
