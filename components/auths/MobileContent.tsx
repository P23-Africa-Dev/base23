...
export default function MobileTopContent({ step, steps, content }: MobileTopContentProps) {
    ...
    return (
        ...
        <div className="h-[260px] w-full md:hidden relative overflow-hidden">
            <img src={images.topFormBg} className="absolute inset-0 z-0 h-auto w-full object-top" alt="" />

            {/* Top content */}
            <div className="absolute top-5 left-6 right-6 z-10 py-2">
                <div className="bg-transparent text-secondaryWhite">
                    <h2 className={`mb-3 font-semibold ${currentContent.headingClassName} `}>{currentContent.title}</h2>
                    <p className={`text-[13px] ${currentContent.paragraphClassName}`}>{currentContent.description}</p>
                </div>
                ...
                {isMultiStep && (
                    <>
                        <div className="mt-5 flex items-center space-x-3 pb-1">
                            {steps.map((_, index) => {
                                const stepNumber = index + 1;
                                const isActive = step >= stepNumber;
                                ...
                                return (
                                    <span
                                        key={stepNumber}
                                        className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full border-2 border-white text-xs font-semibold sm:h-10 sm:w-10 md:text-sm ${
                                            isActive ? 'bg-white text-primary' : 'bg-transparent text-white'
                                        }`}
                                    >
                                        {stepNumber}
                        ...
                        <div className="text-sm font-extralight text-white mt-1">
                            Step {step}/{steps.length}
                        </div>
                    </>
                )}
