...
import RecommendedLeadCard from '@/components/cards/RecommendedLeadCard';
...
import MessageStatsChart from '@/components/chart/MessageStatsChart';
...
function Dashboard() {
    ...
    const [recommendedLeads, setRecommendedLeads] = useState<User[]>([]);
    ...
    useEffect(() => {
        axios.get('/api/dashboard-data').then((res) => {
            ...
            setRecommendedLeads(res.data.recommendedLeads || []);
    ...
    const [isLoadingSortedUsers, setIsLoadingSortedUsers] = useState(false);
    ...
    const [isLoadingMessageStats, setIsLoadingMessageStats] = useState(false);

    // Leads state for Recommended Leads section
    interface LeadData {
        id: number;
        type: string;
        logo: string;
    ...
    const [recommendedLeadsData, setRecommendedLeadsData] = useState<LeadData[]>([]);
    const [isLoadingLeads, setIsLoadingLeads] = useState(false);

    // Add a small delay before making API calls to let CSRF token settle after page navigation
    useEffect(() => {
        const timer = setTimeout(() => {
            axios
                ...
                .catch((error) => {
                    // Silently handle - 419 errors are handled by axios interceptor
    ...
    // Check for subscription success message from localStorage
    useEffect(() => {
        ...
        if (subscriptionSuccess === 'true') {
            ...
            toast.success('🎉 Welcome to NOEL! Your subscription is now active. Enjoy your 14-day free trial!', {
                duration: 6000,
                style: {
                    background: '#024E44',
                    color: '#fff',
    ...
    // Load intelligently sorted users for the dashboard with a small delay
    useEffect(() => {
        if (auth.user) {
            const timer = setTimeout(() => {
                loadIntelligentlySortedUsers();
            }, 500); // 500ms delay
    ...
    // Load recommended leads from the leads API
    useEffect(() => {
        const loadRecommendedLeads = async () => {
            setIsLoadingLeads(true);
            try {
                const response = await axios.get('/api/leads');
                if (response.data.success && response.data.leads) {
                    // Take up to 6 leads for the recommended section
                    setRecommendedLeadsData(response.data.leads.slice(0, 6));
                }
            } catch (error) {
                console.error('Error loading recommended leads:', error);
            } finally {
        ...
        const timer = setTimeout(() => {
            loadRecommendedLeads();
        }, 600); // Slight delay after other API calls

        return () => clearTimeout(timer);
    }, []);

    // Use custom hook for message stats with real-time updates
    const { refreshStats } = useMessageStats({
        onStatsUpdate: setMessageStats,
        refreshInterval: 30000, // 30 seconds
        enabled: !!auth.user,
    });

    const loadIntelligentlySortedUsers = async () => {
        setIsLoadingSortedUsers(true);
        try {
            const response = await axios.post('/api/sort/users', {
            ...
            if (response.data.success) {
                setSortedUsers(response.data.data.users);
            } else {
                // Fallback to original users if intelligent sorting fails
                setSortedUsers(users.filter((user) => user.id !== auth.user?.id));
            }
        } catch (error) {
    ...
    const formatExperience = (years: string | number | null | undefined): string => {
        ...
        if (typeof years === 'string') {
            ...
            return years; // Return as is if can't parse
        }
    ...
    const handleSearch = async () => {
        ...
        setIsSearching(true);
        try {
            // Navigate to directory page with search results
    ...
    const [activeTab, setActiveTab] = useState<'connection' | 'smart-match' | 'active-lead'>('connection');
                ...
                >
                    <div className="relative z-[10] no-scrollbar flex h-screen max-h-[96vh] w-full flex-col gap-3 overflow-y-auto px-2 pb-1 lg:py-0 lg:pr-9 lg:pl-7 xl:pr-17 xl:pl-12">
                        ...
                        <div className="relative mt-1.5 w-full bg-no-repeat px-2 pt-5 pb-4 lg:hidden animate-fadeIn">
                            ...
                            <div className="relative z-[2] grid w-full grid-cols-2 place-content-between px-5 sm:px-12">
                                    ...
                                    {auth.user ? (
                                        <>
                                            <h3 className="text-[14px] font-semibold tracking-wide whitespace-nowrap">
                                                Hello <span className="font-bold tracking-tight"> {auth.user.name}!</span> {/*   */}
                                            </h3>
                                ...
                                <div className="flex items-center gap-2 place-self-end">
                                    <img src={images.notificationsnooze} className="h-4.5 w-auto" alt="" />
                        ...
                        <div className="grid auto-rows-min gap-4 lg:grid-cols-3 stagger-children">
                            {/* CHART CONTAINER */}
                            <div className="grid-card-shadow relative hidden aspect-auto overflow-hidden rounded-2xl bg-gradient-to-r from-[#A47AF0] to-[#A47AF0]/60 p-2 lg:block hover-lift">
                                    ...
                                    <div className="flex justify-between">
                                        ...
                                        <div>
                                            <h5 className="flex items-center justify-end gap-1">
                                                <span
                                                    className={`text-xl leading-10 font-medium lg:text-sm xl:text-base ${activityChange.isIncrease ? 'text-green-600' : 'text-red-600'
                                                        }`}
                                                >
                                    ...
                                    <div className="flex justify-between">
                                        <div className="-mt-4 flex items-center divide-x divide-gray-200">
                                            <div className="xl:pr-5">
                                                <span className="font-bold text-[#F05831] lg:text-lg xl:text-3xl">
                                                    {' '}
                                                    <Counter end={45000} />
                                                </span>{' '}
                                                <br />
                                                <h6 className="text-[#727677] lg:text-sm">Leads</h6>
                                            </div>
                                            <div className="pl-5">
                                                <span className="font-bold text-[#0496FF] lg:text-lg xl:text-3xl">
                                                    <Counter end={connected?.length || 0} />
                                                </span>{' '}
                                                <br />
                                                <h6 className="text-[#727677] lg:text-sm">Connections</h6>
                                            </div>
                                        </div>
                                        ...
                                        <div className="mt-2">
                                            <button
                                                ...
                                                className="flex h-[33px] w-[33px] items-center justify-center rounded-sm bg-[#4C6FFF]"
                                            >
                                                <div className="relative h-6 w-[25px]">
                                                    <img src={images.filedownload} className="absolute h-full w-full object-center" alt="" />
                                                </div>
                                            </button>
                                        </div>
                                    </div>
                            ...
                            <div className="flex justify-between lg:hidden animate-fadeInUp animate-delay-100">
                                <div className="w-full max-w-[290px] md:max-w-[350px]">
                                    <h5 className="mb-1.5 pl-6 text-[10px] font-semibold tracking-wide md:text-base">Network Stats</h5>

                                    <div className="flex rounded-full border-2 border-primary/70 px-4 py-2 pb-2.5 ">
                                        {/* Leads */}
                                        <div className="grid grid-cols-2 place-items-center gap-2">
                                            <img src={images.shareKnowledge} className="md::w-8 h-6 w-6 md:w-8" alt="" />
                                            ...
                                            <div className="leading-4">
                                                ...
                                                <h6 className="text-[10px] text-[#727677] md:text-base">Leads</h6>
                                            </div>
                                        </div>
                                        <div className="grid grid-cols-2 place-items-center">
                                            ...
                                            <div className="leading-4">
                                                ...
                                                <br />
                                                <h6 className="text-[10px] text-[#727677] md:text-base">Connections</h6>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="mt-6 flex items-center justify-items-end gap-1">
                                    <div className="flex flex-col items-center justify-center">
                                        ...
                                        <h4 className="mb-1.5 w-full flex-1 text-center text-[8px] whitespace-nowrap text-primary">200k+ People</h4>
                            ...
                            <div className="block lg:hidden animate-fadeInUp animate-delay-300">
                                ...
                                <Tabs defaultValue="connection" onValueChange={(val) => setActiveTab(val as typeof activeTab)}>
                                        ...
                                        <div className="relative no-scrollbar overflow-y-auto bg-transparent pb-10 lg:px-2">
                                            <div className="sticky top-0 z-10 flex w-full items-center justify-between overflow-hidden rounded-3xl border-b-0 bg-deepBlack px-3 pt-4 pb-3">
                                                <div className="flex w-[27%] flex-col text-white italic xl:w-[160px]">
                                                    ...
                                                    <h3 className="text-[12px] font-extrabold sm:text-xl"> next deal</h3>
                                                </div>
                                                ...
                                                <div className="flex w-[57%] items-center space-x-2">
                                                    <div className="relative w-full cursor-pointer">
                                                        ...
                                                        >
                                                            ...
                                                            <img src={images.aiSearch} className="h-4.5 w-4.5" alt="AI Search" />
                                                        </button>
                                                        {isSearching && (
                                            ...
                                            <TabsContent value="connection" className="mt-0">
                                                ...
                                                variant="all"
                                                                    ...
                                                                    industry={user.industry || 'Industry not specified'}
                                                                    interest={formatArrayData(user.categories)}
                                                                    reviews={user.rating ? `${user.rating}/5` : 'No rating'}
                                                                    baseLocation={user.country || 'Location not specified'}
                                                                    operatesIn={user.country || 'Location not specified'}
                                                                    bio={user.company_description || 'No bio available'}
                                                                    companyStage={user.selected_outcome || 'Stage not specified'}
                                                        ...
                                                        <div className="flex items-center justify-center py-8">
                                                            <div className="text-center text-white">
                                                                ...
                                                                <p className="text-[10px] text-gray-300">Check back later for new connections</p>
                                                            </div>
                                                        </div>
                                                    )}
                                                </div>
                                            ...
                                            <TabsContent value="smart-match" className="mt-0">
                                                ...
                                                variant="all"
                                                        ...
                                                        <div className="flex items-center justify-center py-8">
                                                            <div className="text-center text-white">
                                                                <p className="text-sm font-medium">No smart matches yet</p>
                                                                <p className="text-xs text-gray-300">Complete your profile to get better matches</p>
                                                            </div>
                                                        </div>
                                                    )}
                                                </div>
                                            ...
                                            <TabsContent value="active-lead" className="mt-0">
                                                ...
                                                variant="all"
                                                        ...
                                                        <div className="flex items-center justify-center py-8">
                                                            <div className="text-center text-white">
                                                                <p className="text-xs font-medium">No active leads</p>
                                                                <p className="text-[10px] text-gray-300">Start connecting to generate leads</p>
                                                            </div>
                                                        </div>
                                                    )}
                                                </div>
                        ...
                        {/* SECOND ROW */}
                        <div className="hidden auto-rows-max gap-3 lg:grid lg:grid-cols-5 animate-fadeIn animate-delay-200">
                            {/* First child Show Next Leads spans 2 columns */}
                            <div className="relative col-span-3 -mt-1 aspect-auto h-[63vh] w-full overflow-hidden rounded-3xl bg-deepBlack bg-cover bg-center bg-no-repeat p-3 pb-20 lg:-ml-1.5 lg:bg-transparent lg:pr-12 lg:pl-12 xl:pr-14 hover-lift">
                                <img
                                ...
                                <div className="relative no-scrollbar flex-1 overflow-y-auto px-2 pb-10 lg:pb-0">
                                    {/* <div className="lg:max-h-auto relative no-scrollbar overflow-hidden overflow-y-auto bg-transparent pb-10 lg:px-2 lg:pb-0"> */}
                                    {/* Search Header */}

                                    <div className="sticky top-0 z-10 flex w-full items-center justify-between overflow-hidden border-b-0 bg-deepBlack px-3 pt-4 pb-3 lg:border-b-1 lg:bg-white lg:px-0">
                                        <div className="flex w-[27%] flex-col text-white italic lg:text-deepBlack xl:w-[160px]">
                                            <h2 className="text-[12px] leading-2 font-normal sm:text-[14px] md:text-[15px] lg:text-[17px] lg:leading-3">
                                                {' '}
                                                Let's find your
                                            </h2>
                                            <h3 className="text-base font-extrabold sm:text-xl lg:text-[25px]"> next deal</h3>
                                        </div>
                                        ...
                                        <div className="flex w-[57%] items-center space-x-2 lg:mr-14 lg:w-[60%] lg:items-start">
                                            <div className="relative w-full cursor-pointer">
                                                ...
                                                <button
                                                    ...
                                                    className="absolute top-1/2 right-5 -translate-y-1/2 disabled:cursor-not-allowed"
                                                >
                                                    <img src={images.desktopSearch} className="hidden h-6 w-6 lg:right-10 lg:block" alt="Search" />
                                                    <img src={images.aiSearch} className="h-7 w-7 lg:right-10 lg:hidden" alt="AI Search" />
                                                </button>
                                                {isSearching && (
                                                    <div className="absolute top-1/2 right-12 -translate-y-1/2">
                                    ...
                                    <div className="h-[40vh] divide-y divide-white/30">
                                        {isLoadingSortedUsers ? (
                                            <div className="flex items-center justify-center py-8">
                                                <div className="text-white lg:text-deepBlack text-base">Loading intelligently sorted users...</div>
                                            </div>
                                        ) : (
                            ...
                            <div className="grid-card-shadow relative col-span-2 hidden aspect-auto h-[60vh] overflow-hidden rounded-3xl bg-white lg:block hover-lift">
                                <div className="flex h-full flex-col p-4 pl-12">
                                    <div className="mb-2 flex items-center justify-between">
                                        <h2 className="text-lg font-semibold text-[#414D55]">Message Stats</h2>
                                    </div>

                                    <div className="relative mr-10 flex h-full flex-1 items-center justify-center">
                                        <img
                                            src={images.lineCHart}
                                            alt="charts background"
                                            className="absolute inset-1 h-full w-full object-contain opacity-60"
                                        />

                                        {/* Dynamic Message Stats Chart */}
                                        <div className="relative h-[40vh] w-full">
                                            <MessageStatsChart messageStats={messageStats} isLoading={isLoadingMessageStats} />
                                        </div>
                                    </div>

                                    {/* Unread notification if any */}
                                    {messageStats.unread_messages > 0 && (
                                        <div className="mt-2 flex justify-center">
                                            <span className="rounded bg-red-100 px-2 py-1 text-xs font-medium text-red-600">
                                                {messageStats.unread_messages} unread
                                            </span>
                                        </div>
                                    )}
                                </div>
                        ...
                        {/* THIRD ROW */}
                        <div className="-mt-3 hidden auto-rows-min gap-4 md:grid-cols-5 lg:grid animate-fadeInUp animate-delay-300">
                            {/* SMART CLIENT */}
                            <div className="grid-card-shadow relative col-span-2 aspect-auto overflow-hidden rounded-2xl border-2 border-darkGreen bg-white hover-lift hover-glow">
                            ...
                            <Link href={'/leads'} className="grid-card-shadow relative col-span-3 aspect-auto overflow-hidden rounded-2xl bg-white hover-lift hover-glow">
                                <div className="mx-auto max-w-[650px]  pt-4 xl:px-0">
                                    {/* Header */}
                                    <div className="mb-3 flex items-center justify-between px-3 md:px-0">
                                        ...
                                        <div className="rounded bg-[#59AFD6] px-4 py-0.5 text-[12.5px] font-extralight text-white italic hover:bg-[#59AFD6]/80 dark:bg-blue-900">
                                            Verified
                                        </div>
                                    </div>
                                    ...
                                    <div className="no-scrollbar max-h-[30vh] space-y-4 overflow-y-auto pr-2">
                                        ...
                                        ) : recommendedLeadsData && recommendedLeadsData.length > 0 ? (
                                            recommendedLeadsData.map((lead) => (
                                                <RecommendedLeadCard
                                                    key={lead.id}
                                                    name={lead.name}
                                                    email={lead.email}
                                                    position={lead.position}
                                                    industry={lead.company_name}
                                                    rating={typeof lead.rating === 'string' ? parseFloat(lead.rating) : lead.rating}
                                                    logo={lead.logo}
                                                    type={lead.type as 'person' | 'company'}
                                                />
                                            ))
                                        ) : (
                                            <div className="py-8 text-center text-gray-500">
                                                <p>No recommended leads found.</p>
