...
export default function StepFourForm({ onNext }: StepFourProps) {
  ...
  return (
    <div className="w-full px-5 pr-10 mt-15 md:mt-0  md:p-8 lg:overflow-y-auto xl:ml-16">
      <div className="relative z-10 mx-auto max-w-md xl:max-w-lg">
        {/* Heading */}
        <div className="mb-15">
          <h2 className="mb-1 text-3xl font-extrabold text-primary dark:text-black">
            Control your visibility
          </h2>
          <p className="pr-10 text-[17px] font-normal text-primary dark:text-black">
        ...
        <form onSubmit={handleSubmit} className="space-y-12">
          ...
          <div className="space-y-2">
            {VISIBILITY_OPTIONS.map((opt, idx) => (
              <div
                key={opt.title}
                className="flex items-center justify-between rounded-xl py-4"
              >
                <div className="flex-1">
                  <div className="text-base font-semibold text-primary dark:text-black">
                    {opt.title}
                  </div>
                  <div className="mt-1 max-w-xs pr-5 text-base font-light text-[#8C9AA6]">
                    {opt.desc}
                  </div>
                </div>
          ...
          <div className="flex flex-col items-center lg:mr-10">
            <button
              type="submit"
              className="w-full rounded-2xl bg-pinkLight py-5 font-semibold text-white hover:bg-pinkLight/90"
            >
              Proceed
            </button>

            <div className="mt-6 text-left text-primary md:hidden lg:px-0">
              <p className="text-sm">
