...
import { ImageOptimizationError, optimizeImageToMaxSize, validateImageForOptimization } from '@/utils/imageOptimizer';
...
export default function StepTwoForm({ defaultValues, onNext, restoredProfilePic, restoredProfilePicPreview }: Step2Props) {
    const [preview, setPreview] = useState<string | null>(restoredProfilePicPreview || null);
    const [optimizing, setOptimizing] = useState(false);
    const [selectedFile, setSelectedFile] = useState<File | null>(restoredProfilePic || null);
    ...
    return (
        <div className="overflow-hidden w-full">
            ...
            >
                <div className="mx-auto max-w-[650px]">
                    ...
                    <div className="mb-10 lg:pt-5">
                        <h2 className="mb-1 text-2xl font-extrabold text-primary lg:text-3xl dark:text-black">About your company</h2>
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="mr-2 space-y-7 px-3 md:px-0">
                                ...
                                <input
                                    ...
                                    onChange={async (e) => {
                                        ...
                                        if (validation.needsOptimization) {
                                            setOptimizing(true);
                                            toast.dismiss();
                                            const progressToastId = toast.loading('Optimizing large image... This may take a few seconds.', {
                                                duration: 30000,
                                            });
                                                ...
                                                if (error instanceof ImageOptimizationError) {
                                                    switch (error.code) {
                                                        case 'TOO_LARGE':
                                                            ...
                                                            setFileError('Image is too large. Maximum size is 20MB.');
                                                            break;
                                                        case 'OPTIMIZATION_FAILED':
                                                            toast.error('Could not optimize image. Please try a different image.');
                                                            setFileError('Image optimization failed. Try a smaller or less complex image.');
                                                            break;
                                                        default:
                                                            toast.error(error.message);
                                ...
                                {!preview ? (
                                    <div className="flex w-full flex-col items-center justify-center">
                                        ...
                                        <span className="mt-1 block px-3 text-center text-[7px] text-primary/50 dark:text-black">
                                            {optimizing ? 'Optimizing image...' : 'Click to add profile picture'}
                                        </span>
                                    </div>
                                ) : (
                                    <div
                        ...
                        <div className="relative w-full max-w-[610px] lg:max-w-[620px]">
                            <div className="mt-10 mb-5 flex flex-row gap-6 lg:mb-2 lg:flex-row lg:justify-between lg:gap-0">
                                ...
                                <div className="flex flex-col text-left lg:w-[180px]">
                                    <h2 className="text-[15.5px] font-bold tracking-wider text-darkBlue lg:tracking-wide">Industry</h2>
                                    <p className="text-[12px] font-normal tracking-wider text-[#8C9AA6]">
                            ...
                            <div className="flex flex-col justify-between gap-8 pb-4 lg:flex-row">
                                <div className="w-full">
                                    ...
                                    <div className="relative no-scrollbar h-[200px] w-full rounded-lg border border-[#8C9AA6] leading-relaxed lg:h-[210px] lg:w-[90%] lg:max-w-sm">
                                        ...
                                        >
                                            <HiOutlineSparkles className={`h-3 w-3 ${isRegenerating ? 'animate-spin' : ''}`} />
                                            <span>Rewrite</span>
                                        </button>
                                ...
                                <div className="flex flex-col gap-5 lg:gap-8">
                                    <div className="flex flex-col text-left lg:text-left">
                                        ...
                                        <p className="text-[12px] font-normal text-[#8C9AA6]">{defaultValues.country || 'Not provided'}</p>
                                    </div>
                                    <div className="flex flex-col text-left lg:text-left">
                                        <h2 className="text-[15.5px] font-bold tracking-wider whitespace-nowrap text-darkBlue lg:text-[14.5px]">
                                            Numbers of Employee
                                    ...
                                    <div className="flex flex-col text-left lg:text-left">
                                        <h2 className="text-[15.5px] font-bold tracking-wider text-darkBlue lg:text-[14.5px]">Year Founded</h2>
                                        <p className="text-[12px] font-normal text-[#8C9AA6]">{calculatedYearEstablished || 'Not provided'}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <img src={images.bottomFormBgP} className="absolute md:hidden top-[58%] z-2 h-auto w-full object-cover" alt="" />
        </div>
    );
}
