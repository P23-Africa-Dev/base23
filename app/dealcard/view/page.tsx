...
function DealCardViewContent({ auth, dealCards = [], users = [], stats, activeTab: initialTab = 'viewed', connected = [], pending = [] }: Props) {
...
export default function DealCardView(props: Omit<Props, 'auth'>) {
    const { user } = useAuth();
    if (!user) return null;
    return <DealCardViewContent {...props} auth={{ user }} />;
}
